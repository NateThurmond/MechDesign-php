...
include("php/DB_Conn.php");
    ...
    //if ( isset($_GET['incArm']) ) {    $incArm = $_GET['incArm'];    }
    ...
    if ($incArm == 'decrease') {
        ...
        function updateArmor($armorLocation, $armorLocationMod, $linked, $altArmorLocation, $mechIdPassed, $conn) {
            $retval2 = mysqli_query($conn, "UPDATE mechexternalarmor SET $armorLocation = $armorLocation+$armorLocationMod WHERE mechID = $mechIdPassed");        
                if(! $retval2 ) { die('Could not update data: ' . mysqli_error($conn)); } echo "Updated data successfully\n";
            ...
            if ($linked == 'yes') {
                ...
                if(! $retval4 ) { die('Could not update data: ' . mysqli_error($conn)); } echo "Updated data successfully\n";
            }
        }
        ...
        if(! $retval3 ) { die('Could not update data: ' . mysqli_error($conn)); } echo "Updated data successfully\n";
